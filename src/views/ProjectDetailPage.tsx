...
const ProjectVisual = ({ image, title, type }: { image: string; title: string; type: string }) => (
  <div className="w-full h-full relative overflow-hidden">
    <img src={image} alt={title} className="w-full h-full object-cover" />
    <div className="absolute inset-0 bg-gradient-to-t from-black/45 via-black/20 to-black/10" />
    <div className="absolute bottom-5 right-5 z-10 bg-black/35 backdrop-blur-sm rounded-xl px-3 py-2 text-white">
      <p className="text-xs text-white/80 mb-0.5">{type}</p>
...
export default function ProjectDetailPage({
  ...
  const SHOW_AVAILABLE_UNITS_SECTION = false;
  const params = useParams<{ slug: string }>();
  const rawSlug = slugProp ?? params?.slug ?? '';
  const normalizedSlug = decodeURIComponent(String(rawSlug)).trim().normalize('NFC');
  const [project, setProject] = useState<Project | null>(initialProject ?? null);
  const [loadingProject, setLoadingProject] = useState(!initialProject);
  ...
  useEffect(() => {
    ...
    getProjectBySlug(normalizedSlug).then((item) => {
      if (!active) return;
  ...
  const galleryImages = project?.images?.length ? project.images : fallbackGalleryImages;
  const galleryVideos = [
    ...
    return (
      <div className="min-h-screen pt-20 flex items-center justify-center">
        <div className="text-center">
          ...
          <Link href="/projects" className="btn-primary px-6 py-3 rounded-xl font-bold text-sm">
            العودة للمشاريع
          </Link>
        </div>
      </div>
  ...
  return (
    <div className="min-h-screen pt-20 pb-24 lg:pb-0">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            >
              <ProjectVisual image={galleryImages[0]} title={project.title} type={project.type} />
            </motion.div>
            ...
            >
              ...
              <div className="flex flex-wrap gap-3">
                {project.files?.length ? (
                  ...
                  <p className="text-sm text-gray-500">لا توجد ملفات متاحة لهذا المشروع حالياً.</p>
                )}
              </div>
            </motion.div>
          </div>
          ...
          <div className="lg:sticky lg:top-24 self-start h-fit">
            <motion.div
              ...
              transition={{ delay: 0.2 }}
              className="bg-white rounded-2xl p-6 border border-[var(--color-border)] shadow-[var(--shadow-soft)]"
            >
              ...
              <div className="border-t border-gray-100 pt-5">
                ...
                <ContactForm propertyTitle={project.title} compact target={{ type: 'project', id: project.id }} />
              </div>
            </motion.div>
          </div>
        </div>
      ...
      <div className="lg:hidden fixed bottom-0 inset-x-0 z-40 bg-white/95 backdrop-blur border-t border-gray-200 p-3">
        <div className="flex items-center gap-2">
          <a
            href="https://wa.me/966500000000"
            target="_blank"
