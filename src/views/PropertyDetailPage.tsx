...
import { getPropertyBySlug } from '../lib/content-service';
...
const PropertyVisual = ({ image, type, title }: { image: string; type: string; title: string }) => (
...
export default function PropertyDetailPage({
  ...
  const [loadingProperty, setLoadingProperty] = useState(!initialProperty);
  ...
  useEffect(() => {
    ...
    getPropertyBySlug(normalizedSlug).then((item) => {
      if (!active) return;
  ...
  const galleryImages = property?.images?.length ? property.images : fallbackGalleryImages;
  const galleryVideos = [
    ...
    return (
      <div className="min-h-screen pt-20 flex items-center justify-center">
        <div className="text-center">
          ...
          <Link href="/properties" className="btn-primary px-6 py-3 rounded-xl font-bold text-sm">
            العودة للعقارات
          </Link>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            >
              <PropertyVisual image={galleryImages[0]} type={property.type} title={property.title} />
            </motion.div>
            ...
            >
              ...
              <div className="flex flex-wrap gap-3">
                {property.files?.length ? (
                  ...
                  <p className="text-sm text-gray-500">لا توجد ملفات متاحة لهذا العقار حالياً.</p>
                )}
              </div>
            </motion.div>
          ...
          <div className="space-y-6 lg:sticky lg:top-24 self-start h-fit">
            ...
            >
              ...
              <ContactForm propertyTitle={property.title} compact target={{ type: 'property', id: property.id }} />
            </motion.div>
          </div>
        </div>
      </div>
