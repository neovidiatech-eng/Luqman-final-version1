...
import { type BlogPost, type Project, type Property } from '../data/properties';
import { getBlogPosts, getProjects, getProperties } from '../lib/content-service';
...
export default function HomePage() {
  ...
  useEffect(() => {
    let active = true;
    Promise.all([getProperties(), getProjects(), getBlogPosts()]).then(([propertyItems, projectItems, postItems]) => {
      if (!active) return;
      if (propertyItems.length > 0) setProperties(propertyItems);
      if (projectItems.length > 0) setProjects(projectItems);
      if (postItems.length > 0) setPosts(postItems);
    });
    return () => {
      active = false;
    };
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
            ...
            <div>
              ...
              <div className="mt-6">
                <Link href="/about" className="btn-primary px-6 py-3 rounded-xl font-bold text-sm inline-flex items-center gap-2">
                  اعرف أكثر عن لقمان
                  <ArrowLeft className="w-4 h-4" />
                </Link>
              </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-end justify-between mb-12">
            <SectionHeader
              ...
              align="right"
            />
            <Link href="/properties" className="text-primary font-bold text-sm flex items-center gap-1 hover:gap-2 transition-all">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-end justify-between mb-12">
            <SectionHeader
              ...
              align="right"
            />
            <Link href="/projects" className="text-primary font-bold text-sm flex items-center gap-1 hover:gap-2 transition-all">
              عرض الكل <ArrowLeft className="w-4 h-4" />
            </Link>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
